// API key for accessing contents from the original website (Unsplash) //
const accessKey = "qZ31D6MpZd2aSa0A5Ej07EhyQom1hdt4-rmYB53h7Ws";
// variable defining for accessing HTML elements //
...
let page = 1;
// asyncronus JavaScript function to fetch API //
async function searchImages() {
  //fetch user input //
  inputData = inputEl.value;
  // dynamic url to fetch data from the origial website using API key //
  const url = `https://api.unsplash.com/search/photos?page=${page}&query=${inputData}&client_id=${accessKey}`;

  const response = await fetch(url); //method to fetch based on the query //
  const data = await response.json(); //data converted into json Object //
  ...
  results.map((result) => {
    const imageWrapper = document.createElement("div"); //Dynamic Container creation //
    imageWrapper.classList.add("search-result"); // will push the result //
    ...
    const imageLink = document.createElement("a"); // Dynamic <a> element creation //
